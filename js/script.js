 ...
 * 23. 動作保証: オープニング(ロゴ/プロローグ)〜タイトル〜バトル開始のいずれも、画像読み込み完了を待たせない。ゲームループ(draw)とUIインタラクションの確定(boot)は、ページ読み込み後ただちに行う。バトルで使うキャラ画像(DB.ASSETS)・背景に加え、プロローグの画像(OPENING_SCREENS、op_1〜4.PNG)も、ページ読み込み直後から裏で並行して読み込みを進める。プロローグの画像は以前playPrologue()が呼ばれた時点(ロゴ演出の後)で初めて読み込みを開始しており頭出しの猶予が無かったが、DB.ASSETSと同様にページ読み込み直後から先読みすることで、ロゴ画面が表示されている数秒間を読み込みの猶予時間として使う(playPrologue()側の読み込み待ちは、この先読みでキャッシュ済みのPromiseを再利用するだけになるため、通常は即座に解決する)。バトル開始(STORY MODEのgoBattleStart、TRAINING MODEのgoTrainingBattle)自体はDB.ASSETS等の読み込みを待たない。ロゴ〜プロローグ〜タイトル〜(STORY MODEはデッキ編成)を経てバトル開始に至るまでの導線、およびバトル開始演出(暗転→フェードイン→背景演出→BATTLE START)自体の数秒間が、実質的な読み込みの猶予時間になる。万一この間に読み込みが間に合わなかった場合のみ、draw()側の既存フォールバック表示(緑/赤の四角)が一時的に使われ、読み込みが完了し次第そのフレームから自動的に実画像へ切り替わる(あくまで保険であり、通常発生する想定ではない。バトル開始時に専用のローディング画面を挟むことはしない)。
