...
function checkUnlockAnnouncements() {
    if ((unlockedSubStories.length > 0 || soundTestUnlocked) && !bonusContentsAnnounced) {
...
async function runUpperCombo(attacker, defender, cursor) {
    ...
    // 次の手がGUARDであれば常にこの演出になる)。攻撃側はdashで元の位置には戻らず、素早く着地して次のGUARDの構えを先取りする。
    // 被弾側は無防備なままdamage.PNGの姿勢を保ち、重力に従うように加速しながら(ふわりとした着地の余韻付きで)地面へ落ちる。
    if (nextQueuedMove(attacker, cursor) === 'GUARD') {
        const attackerFromY = getY(attacker), defenderFromY = getY(defender);
        const steps = 10, stepMs = 35;
        for (let s = 1; s <= steps; s++) {
            const t = s / steps;
            setY(attacker, attackerFromY + (DB.POS.GROUND_Y - attackerFromY) * Math.min(1, t * 1.8)); // 攻撃側は先に着地を終える
            setY(defender, defenderFromY + (DB.POS.GROUND_Y - defenderFromY) * (t * t)); // 被弾側は重力っぽく加速しながら落下(t^2のイージング)
            await wait(stepMs);
        }
        setY(attacker, DB.POS.GROUND_Y);
        setAct(attacker, 'guard.PNG'); // 次に控えるGUARDの構えを先取りする
        // 被弾側は着地の瞬間、ふわりと弾む余韻を付ける(小さく数回バウンドして収まる)
        const bounceSteps = 4, bounceStepMs = 45;
        for (let s = 1; s <= bounceSteps; s++) {
            const bounce = Math.abs(Math.sin(s / bounceSteps * Math.PI * 2)) * 10 * (1 - s / bounceSteps);
            setY(defender, DB.POS.GROUND_Y - bounce);
            await wait(bounceStepMs);
        }
        setY(defender, DB.POS.GROUND_Y);
        setAct(defender, 'damage.PNG'); // 無防備なまま
        await wait(150);
        return;
    }
...
function updateBonusContentsUI() {
    const anyUnlocked = unlockedSubStories.length > 0 || soundTestUnlocked;
    ...
    if (soundRow) soundRow.style.display = soundTestUnlocked ? 'flex' : 'none';
}
function openBonusContents() {
    updateBonusContentsUI();
    document.getElementById('bonusContentsOverlay').classList.add('show');
}
function closeBonusContents() { document.getElementById('bonusContentsOverlay').classList.remove('show'); }
